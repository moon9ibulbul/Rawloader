...
logln("[runner-mangago] Job $job_id mulai.");
...
$cmd1 = escapeshellcmd(PYTHON_BIN) . ' -u ' . escapeshellarg(SCRIPTS_DIR . '/mangago.py') . ' '
      . escapeshellarg($chapter_url) . ' '
      . escapeshellarg($raw_dir);

update_meta(['stage'=>'download','progress'=>5]);
logln('[runner-mangago] Menjalankan downloader...');
logln($cmd1);
...
if ($proc) {
  ...
  logln('[runner-mangago] ERROR: gagal menjalankan downloader');
  update_meta(['stage'=>'error','progress'=>100,'error'=>'Downloader failed to start']);
  exit(1);
}

if ($rc1 !== 0) {
  logln('[runner-mangago] ERROR: downloader exit code ' . $rc1);
  update_meta(['stage'=>'error','progress'=>100,'error'=>'Downloader failed']);
  exit(1);
}

function dir_has_images($dir) {
  if (!is_dir($dir)) return false;
  $exts = ['jpg','jpeg','png','webp','bmp','tiff','tga','JPG','JPEG','PNG','WEBP'];
  foreach ($exts as $e) {
    $found = glob($dir . '/*.' . $e);
...
if ($subs) {
  ...
  logln('[runner-mangago] raw subdirs: ' . implode(', ', $names));
}

if (!$input_for_stitch) {
  logln('[runner-mangago] ERROR: Tidak menemukan gambar setelah download');
...
logln('[runner-mangago] Mengemas hasil menjadi ZIP...');
...
logln('[runner-mangago] Selesai. ZIP: ' . basename($zip_path));
